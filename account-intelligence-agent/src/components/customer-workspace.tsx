...
function Overview({ detail, onViewTimeline, onEvidence }: { detail: Detail; onViewTimeline: () => void; onEvidence: (selection: EvidenceSelection) => void }) {
  ...
  return <div className="overview-grid">
    ...
    <article className="experience-card"><SectionTitle icon={<BriefcaseBusiness size={17} />}>客户案例与经验</SectionTitle>{detail.experiences.slice(0, 3).map((item, index) => <div className="case" key={item.id}><span className={`case-icon c${index}`}><Lightbulb size={14} /></span><p><b>{item.title}</b><small>{item.description}</small></p>{item.source_item_ids[0] && <button className="case-evidence" onClick={() => onEvidence({ sourceId: item.source_item_ids[0], conclusion: item.description })}>证据</button>}</div>)}{!detail.experiences.length && <p className="muted">暂无从该客户提炼的案例经验</p>}</article>
...
function Timeline({ events, full = false, onViewAll, onEvidence }: { events: EventItem[]; full?: boolean; onViewAll?: () => void; onEvidence?: (selection: EvidenceSelection) => void }) {
  return <div className={`timeline ${full ? "full-timeline" : ""}`}>
    {events.map((event) => <div className="timeline-row" key={event.id}>
      <time>{dateText(event.occurred_at)}</time>
      <i className={event.importance >= 8 ? "critical" : "normal"}>{event.importance >= 8 ? <CircleAlert size={13} /> : <Check size={12} />}</i>
      <div className="timeline-event-copy"><b>{eventLabel[event.event_type] || event.event_type}</b>{full && <><p>{event.summary}</p><small>{event.source_title}</small></>}</div>
      {!full && <span title={event.payload?.evidence_text || event.summary}>{event.summary}</span>}
      {onEvidence ? <button className="timeline-evidence" onClick={() => onEvidence({ sourceId: event.source_item_id, evidenceText: event.payload?.evidence_text, conclusion: event.summary })}>{sourceTypeLabel[event.source_type] || event.source_type}</button> : <em>{sourceTypeLabel[event.source_type] || event.source_type}</em>}
    </div>)}
    {!events.length && <p className="muted">暂无客户事件</p>}
    {events.length > 0 && !full && <button className="text-button" onClick={onViewAll}>查看完整时间线<ArrowRight size={14} /></button>}
  </div>;
}
...
function Insights({ detail, onEvidence }: { detail: Detail; onEvidence: (selection: EvidenceSelection) => void }) { return <div className="tab-page"><h2>关键洞察</h2><div className="insight-stack"><InfoCard title="成功因素" icon={<ShieldCheck size={17} />} values={detail.success_factors} tone="blue" facts={detail.facts} factTypes={["SUCCESS_FACTOR"]} onEvidence={onEvidence} /><InfoCard title="失败原因" icon={<AlertTriangle size={17} />} values={detail.failure_reasons} tone="red" facts={detail.facts} factTypes={["FAILURE_REASON"]} onEvidence={onEvidence} /></div><InfoCard title="可复用打法" icon={<Lightbulb size={17} />} values={detail.reusable_playbook} tone="blue" facts={detail.facts} factTypes={["PLAYBOOK"]} onEvidence={onEvidence} /></div>; }
function Experiences({ detail, onEvidence }: { detail: Detail; onEvidence: (selection: EvidenceSelection) => void }) { return <div className="tab-page"><h2>客户经验</h2><div className="experience-list">{detail.experiences.map((item) => <article key={item.id}><i><Lightbulb /></i><div><h3>{item.title}</h3><p>{item.description}</p>{item.source_item_ids[0] && <button className="experience-evidence" onClick={() => onEvidence({ sourceId: item.source_item_ids[0], conclusion: item.description })}><FileText size={12} />查看原始材料</button>}</div></article>)}{!detail.experiences.length && <div className="empty-tab"><Lightbulb /><b>暂无客户经验</b><span>系统会从成功、失败材料中提炼可复用经验</span></div>}</div></div>; }
