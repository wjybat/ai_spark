...
顶部“重新扫描市场”是纯前端流程演示，不调用模型。国家页的“生成国家简报”和第一家企业详情里的“生成 BD 作战包”分别启动独立后端流程。

国家流程使用 `scope=country` 与 `countryId`，服务端选择该国三家企业完整档案。Agent 依次读取国家上下文、三家企业证据，再提交模型综合撰写的管理层简报，回答区域优先级、机会逻辑、关键信号、风险、行动与置信度六个问题。每家独立评估、信号覆盖和引用关系均需通过校验，live 失败不会用预览内容冒充结果。

客户流程使用 `scope=customer`，保留原有九步工具链、能力匹配、英文开发邮件、客户七个分页及作战包抽屉。它只分析选中的第一家企业，不覆盖国家简报。后两家作为国家分析样本，客户卡片仍只读。

重新运行期间保留上次成功结果，成功后整体替换，失败不覆盖。结果目前按国家保存在当前页面内存中，刷新后需要重新运行；切换到未运行的国家仍显示调研资料。

前后端共用 `country-brief-data.js` 的 33 份国别企业档案；后端只读取项目固定脚本，不接受请求上传分析资料。新增 `GET /api/countries/:countryId/research` 便于核对三家研究范围。逐国预览答案见 `docs/国家层_管理层简报.md`，运行 `node scripts/export-management-brief.mjs` 可同步更新。

## Qwen / OpenAI 兼容接口

在本目录被 Git 忽略的 `.env` 中配置：

```dotenv
AGENT_MODE=auto
AGENT_PROVIDER=openai-compatible
AGENT_BASE_URL=https://<deployment-id>.cn-beijing.maas.aliyuncs.com/compatible-mode/v1
AGENT_MODEL=qwen3.7-flash
AGENT_THINKING_EFFORT=off
OPENAI_API_KEY=<仅保存在本地 .env>
```
