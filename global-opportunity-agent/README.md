...
前端的最终结论、运行完成弹窗与工作包统一支持 Markdown 标题、列表、表格、引用和代码块。渲染前会清洗 HTML、过滤危险链接，复制和下载保持原始文本不变。
...
运行前，五个分页保留原始调研资料；成功运行后，同一份后端结果分别回填市场与商机（市场雷达、信号、证据）、客户雷达（客户池、画像）、销售建议（行动、拜访问题、邮件）、作战卡（准入、能力匹配、风险）和管理层简报（最终结论、研究摘要、行动）。原有“Agent 实时结果”分页和工作包继续保留。

各页标注运行 ID、完成时间、模式与分析范围。Agent 请求携带当前国家 ID/名称，运行结果和管理层简报以国家为主报告范围；当前公司是首个潜在客户样本，后续客户发现可继续扩充名单。区域评分、集团规模不等同于所选国家的数据。live 模式的能力匹配、英文开发邮件及最终结论由模型生成，其余结构化内容来自证据工具链。管理层简报的显示、复制和 Markdown 下载使用同一份内容。
...
当前测试覆盖三家客户的 P0 4–10、完整 pi Agent 工具顺序、HTTP 创建/完成运行链路，以及五页回填、重新运行更新、跨国家隔离、Markdown 安全渲染和简报复制/下载一致性。
