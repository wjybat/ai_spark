...
export type LiveProvider = "dmall-router" | "openai" | "anthropic";
...
function parseProvider(value: string | undefined): LiveProvider {
  if (value === "anthropic" || value === "openai" || value === "dmall-router") return value;
  return "dmall-router";
}
...
export const config = {
  ...
  thinkingEffort: process.env.AGENT_THINKING_EFFORT === "max" ? "max" as const : process.env.AGENT_THINKING_EFFORT === "xhigh" ? "xhigh" as const : "high" as const,
