...
describe("Agent output surfaces", () => {
  it("shows generated matching and email in their tabs and keeps the same content in exported packages", () => {
    ...
    expect(battle.textContent).toContain("实时智能分析");
    expect(battle.textContent).toContain(productInput.analysis.matches[0]!.pilotScope);
    expect(battle.textContent).not.toContain(productInput.analysis.matches[0]!.caution);
    expect(battle.textContent).not.toContain(productInput.analysis.avoidClaims[0]);
    expect(battle.textContent).not.toContain("而不是直接建议更换电商平台");
    ...
    expect(sales.textContent).not.toContain("优先联系角色");
    expect(sales.textContent).not.toContain("首次拜访问题");
    ...
    expect(artifacts.find(item => item.id === "match").text).not.toContain(productInput.analysis.matches[0]!.caution);
    expect(artifacts.find(item => item.id === "match").text).not.toContain(productInput.analysis.avoidClaims[0]);
