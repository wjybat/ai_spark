...
国家简报与客户作战包独立：程序一次性加载国家及三家企业资料，模型直接综合并提交结构化简报，避免四轮仅为取资料的模型往返。校验不通过时通过 `repair_country_brief` 局部修订，合并后重新执行全部结构和证据检查，不发布未经校验的草稿。具体配置、耗时日志及验证记录见 [国家简报性能](country-brief-performance.md)。
